...
\begin{frame}{2. Narzędzia użyte do realizacji równoległości}
    ...
    \begin{block}{Kluczowe technologie}
        \begin{itemize}
            ...
            \item \textbf{CUDA (Compute Unified Device Architecture):} Platforma obliczeń równoległych na GPU firmy NVIDIA.
...
\begin{frame}{4. Opis eksperymentów (Wariant OCP\_THRIFTY)}
    ...
    \begin{itemize}
        \item \textbf{Problem:} Gra Connect4 (4 w rzędzie).
...
\begin{frame}{Wyniki: Jakość decyzji (Best Q)}
    ...
    \begin{figure}
        \centering
        % Tutaj wstawiamy przesłany obrazek
        \includegraphics[width=0.75\textwidth]{../plots/best_q_boxplot.png}
        \caption{Rozkład wartości Q (wykres pudełkowy). Wyższe N zmniejsza wariancję.}
    \end{figure}
\end{frame}
...
\begin{frame}{Wyniki: Wydajność obliczeniowa}
    ...
    \textbf{Obserwacja:} Wykres pokazuje nieliniowy wzrost. GPU potrzebuje odpowiednio dużego $N$, aby wysycić rdzenie CUDA i ukryć opóźnienia pamięci.
\end{frame}
...
\begin{frame}{Analiza obciążenia GPU (Monitoring)}
    ...
    \begin{itemize}
        \item Średnie obciążenie utrzymuje się na wysokim poziomie (~90\%).
        \item Widoczne "szpilki" (spadki) odpowiadają momentom synchronizacji lub przesyłu danych między CPU a GPU.
    \end{itemize}
\end{frame}
...
\begin{frame}{Propozycja ulepszenia -- Rozwiązanie}
    \begin{block}{Rozwiązanie}
        \begin{itemize}
            \item Wprowadzenie tablicy wag dla głębokości przesyłanej do \textbf{pamięci stałej (constant memory)} GPU.
